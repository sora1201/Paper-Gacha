...
describe("paper book search terms",()=>{
  it("prioritizes relevant OpenAlex topics and uses at most two",()=>{
    ...
    expect(paperBookSearchQuery(paper,["Configured Topic"])).toBe("Graph Neural Networks Molecular Learning");
  });
  ...
  it("works without an abstract and falls back through configured topics to title",()=>{
    ...
    expect(paperBookSearchQuery(generic)).toBe(generic.title);
    expect(paperBookSearchQuery(generic)).not.toBe("");
  });
  ...
  it("extracts a useful Japanese term",()=>{
    ...
    expect(paperBookSearchQuery(paper)).toContain("量子");
  });
});
