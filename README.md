...
Google is the only trusted provider for implicit account linking. When Google returns its verified email and an existing email/password user has that same email, Better Auth attaches the Google account to that existing user rather than creating a second sync identity. `requireLocalEmailVerified` is deliberately disabled because a credential user created while email delivery is disabled has `emailVerified = false`; trust comes from Google's verified identity and an exact email match, not from that local flag. Different email addresses are never implicitly linked (`allowDifferentEmails` remains disabled), other providers are not trusted, and an external account already owned by another user is not moved. No D1 migration or Google Cloud Console change is required for this policy.
