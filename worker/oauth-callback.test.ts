...
const callback = new Request("https://paper-gacha.sora-yamada.workers.dev/api/auth/callback/google?code=test&state=test");
...
describe("Google OAuth callback", () => {
  it("keeps Better Auth's successful settings redirect and session cookie", () => {
    const source = new Response(null, { status: 302, headers: {
      ...
      "set-cookie": "better-auth.session_token=token; Path=/; HttpOnly; Secure; SameSite=Lax",
    } });
  ...
  it("turns a successful non-redirect callback response into /settings", () => {
    const source = new Response("ok", { headers: { "set-cookie": "better-auth.session_token=token; Path=/" } });
    const response = normalizeGoogleCallbackResponse(callback, source, origin);
    expect(response.status).toBe(302);
    expect(response.headers.get("location")).toBe(`${origin}/settings`);
    expect(response.headers.get("set-cookie")).toContain("better-auth.session_token=token");
  });
